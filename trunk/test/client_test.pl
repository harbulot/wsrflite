...
my %CounterTypes = (  'http://localhost:50000/Session/Counter/Counter'  
...
{
  unlink($baseDir."/testlog") or 
        die "Could not remove old result file $baseDir/testlog: $!\n"; 
}
...
chdir($DirPath."/ServiceGroup") or die "Could not change directory to $DirPath/ServiceGroup: $!\n";

print ">>>>>>Creating ServiceGroup>>>>>\n";
my ($ServiceGroupURI);
 eval { $ServiceGroupURI = createServiceGroup("http://localhost:50000/Session/myServiceGroup/myServiceGroup"); };
 if( $@ )
 {
   chdir($baseDir) or die "Could not cd to $baseDir\n";
   print "Test Failed: ".$@;
...
sub runCounterTests {
 ...
 print "Attempting to use GetMultipleResourceProperty on Counter\n $CounterEPR\n"; 
  my ($newcount,$TT);
 ...
 print "Value of count for Counter is $newcount, value of TerminationTime is $TT\n\n";
 $NoPasses++;
...
sub getMultipleResourceProperty {
  ...
  my $cmd = "./$basecmd ".$URL." count TerminationTime";
 ...
 die "Failed in getMultipleResourceProperty, did not get count and Termintation values"
    unless ($i == 2);
...
sub createCounter {
 ...
 my $cmd = "./$basecmd ".$arg{URL}." ".$arg{URI};

 print "\nExecuting...\n \"$cmd 2>> $baseDir/testlog\"\n";
   my $output = `$cmd 2>> $baseDir/testlog`; 
 print $output;  

 die "Failed to create Counter with URL: ".$arg{URL}." and URI ".$arg{URI} 
          if  ( $output eq "" );
...
sub changeTerminationTime {
  ...
  my $cmd = "./$basecmd ".$URL." ".$newTT;
  
  print "\nExecuting...\n \"$cmd 2>> $baseDir/testlog\"\n";
    my $output = `$cmd 2>> $baseDir/testlog`; 
  print $output; 

  die "Failed to set TerminationTime for Counter: ".$URL." and TT".$newTT
          if  ( $output eq "" );
 ...
 die "Failed in changeTerminationTime, times do not match $TerminationTime and $newTT."
...
sub add {
   ...
   my $cmd = "./$basecmd ".$URL." http://vermont/Counter add 1";
   print "\nExecuting...\n \"$cmd 2>> $baseDir/testlog\"\n";
   my $output = `$cmd 2>> $baseDir/testlog`;
   print $output;
