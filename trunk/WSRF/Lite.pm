...
$WSRF::Constants::WSU  = 'http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-utility-1.0.xsd';
$WSRF::Constants::WSSE = 'http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-secext-1.0.xsd';
...
sub envelope {
   ...
   if ( defined($ENV{WSS_SIGN}) )
   {
       ...
       #add them
       my($encoded) = $self->encode_object( SOAP::Data->name(SOAP::Utils::qualify($self->envprefix => 'Envelope') => 
                                   \SOAP::Data->value(SOAP::Data->name(SOAP::Utils::qualify($self->envprefix => 'Header')=> \SOAP::Data->value($header)->type('xml')),
...
sub handle {
  ...
  if (! $r ) { print STDERR "$$ WSRF::Container HTTP::Request not defined!"; return;}
  
  my $Path = $r->uri->path;
  if( $Path =~ m/\.{2,}/og )
  { 
     print STDERR "$$ WSRF::Container Path $Path contains unacceptable charactors.\n";
...
sub WSRFHandler {
   ...
   { 
      ...
      $fail->header( 'Content-Type' => 'text/xml');
      $fail->content(
    	 SOAP::Serializer->fault('Bad WS-Resource Identifier',
  ...
  if ( $request->method eq 'PUT' )
  {
    ...
    chop $To;
    $To .= $request->uri;     
    my $header = SOAP::Header->value("<wsa:To>".$To."</wsa:To>")
                             ->type('xml');			  
    ...
    my $data = SOAP::Data->name('PutResourcePropertyDocument')
                         ->prefix('wsrp')
    ...
    $request->method('POST');
    $request->header( "SOAPAction" => "$WSRF::Constants::WSRP/PutResourcePropertyDocument");
   ...
   if ( $request->method eq 'GET' )
   {
     ...
     #looking for xsl or css or js
     {
       ...
       $resp->header('Content-Type' => 'text/xml') if ($ID =~ m/\.xsl$/) ;
       $resp->header('Content-Type' => 'text/css') if ($ID =~ m/\.css$/) ;
       $resp->header('Content-Type' => 'text/javascript') if ($ID =~ m/\.js$/) ;       $resp->header('Content-Type' => 'text/xml') if ($ID =~ m/\.svg$/) ;
     ...
     $GET = 1;
     my $data = SOAP::Data->name('GetResourcePropertyDocument')
                          ->prefix('wsrp')
   	 	          ->attr( {'xmlns:wsrp' => $WSRF::Constants::WSRP } );
     my $To = $ENV{URL};
     chop $To;
     ...
     $request = HTTP::Request->new();
     $request->method('POST');
     $request->header( "SOAPAction" => "$WSRF::Constants::WSRP/GetResourcePropertyDocument");
     $request->header( "Content-Length" => length $envelope );
     $request->content($envelope);
   }
   ...
   {
     my $data = SOAP::Data->name('Destroy')
                          ->prefix('wsrl')
   ...
   if ( ! -S $rend )
   {
      ...
      $fail->header( 'Content-Type' => 'text/xml');
      $fail->content(
    	 SOAP::Serializer->fault('No such WS-Resource type',
   ...
   my $MyFH = IO::Socket::UNIX->new( Peer => "$rend",
   				     Type => SOCK_STREAM,
   				     Timeout => 10 ) or 
   ...
   if ( defined( $ENV{SSL_CLIENT_DN}) )
   {
       ...
       $request->header( 'Client-SSL-Cert-Issuer' => "$ENV{SSL_CLIENT_ISSUER}" ); 
   }
...
#   print STDERR "$$ >>>out>>>\n".$resp->content."\n<<<out<<<\n";
   ...
   {
      $resp =  WSRF::Container::getProperties($resp, $Directory,$ModuleName);
      $resp->header("Pragma" => "no-cache");
      $resp->header("Cache-Control" => "no-cache, max-age=1, must-revalidate");
   }
   return $resp;      
}
...
sub SessionHandler {
   ...
   my $ID = pop @PathArray;
   my ($module);
   if ( $ID =~ /\d+-?d*/o || $ID =~ /^\w+\.(js|xsl|css|svg)$/ ) #a resource identifier
   {
   ...
   if( ! -f $tmpPath )
   {
     ...
     $fail->content(  
   	SOAP::Serializer->fault('No Service',
   				"Check Endpoint of Service")
   		   );
     return $fail; 
   }
  ...
  if ( $request->method eq 'PUT' )
  {
   ...
   my $resp = WSRF::Session->dispatch_to ($Directory)
   			   ->dispatch_with(\%namespacemap) 
...
sub getProperties {
     ...
     my $Module = shift @_;
     my $xml = $resp->content;
     eval {require XML::CanonicalizeXML};
     if ( ! $@ )  #we have XML::CanonicalizeXML, so we can strip the SOAP stuff
     {
        my $xpath = 
'<XPath xmlns:wsrp="'.$WSRF::Constants::WSRP.'">
(//. | //@* | //namespace::*)[ancestor-or-self::wsrp:ResourceProperties]
</XPath>';
        my $canon = '<?xml version="1.0" encoding="ISO-8859-1"?>'."\n";
        $canon = $canon.'<?xml-stylesheet type="text/xsl" href="'.$Module.'.xsl"?>'."\n"
               if (  -f $Dir."/$Module.xsl" && -r $Dir."/$Module.xsl" );         
        $canon .= XML::CanonicalizeXML::canonicalize($xml,$xpath,'',1,0);
...
sub MultiSessionHandler {
   ...
   if ( $ID =~ /\d+-?d*/o || $ID =~ /^\w+\.(xsl|js|css|svg)$/o) #a resource identifier
   {
      $module = pop @PathArray;
   }
   else
  ...
  if ( $request->method eq 'PUT' )
  {
   ...
   if(  ! -S $SockPath  )
   {
    ...
    my $MyFH = IO::Socket::UNIX->new( Peer => $SockPath,
     ...
     #print "Ingoing HTTP>>>\n".$r->as_string()."\n<<<HTTP\n";
     my $out = print $MyFH ( $request->as_string() );
     if (!defined($out)) {print STDERR "$$ ERROR could not write to $MyFH\n"}
     #read the response from the Socket and turn it into a 
     #HTTP::Response 
   ...
   else    #no process to handle this message - we need to create one
   {
     ...
     if ( ! $MyFH )			       
     {
      ...
      $MyFH = IO::Socket::UNIX->new( Peer => $SockPath,
     				     Type => SOCK_STREAM,
     				     Timeout => 10 ) or 
	    			     die SOAP::Fault->faultcode("Container Fault")
	    					    ->faultstring("Container Failure - Socket problem $!");
...
#  print STDERR "WSRF::Container::GetWSDL ShortPATH=\"$ShortPATH\"\n"; 
  ...
  if (  -r $LongPATH  )
  {
    ...
    if ( $@ )
    {
      print STDERR "$$ WSRF::Container::GetWSDL could not retrieve WSDL from $LongPATH";
  ...
  elsif ( -r $ShortPATH )
  {
    ...
    if ( $@ )
    {
      print STDERR "$$ WSRF::Container::GetWSDL could not retrieve WSDL from $ShortPATH";
      $resp = HTTP::Response->new(RC_INTERNAL_SERVER_ERROR);      
    }	    
  }
  else
...
sub new {
   ...
   if ( defined ($stuff) )
   {
    ...
    my $som = UNIVERSAL::isa($stuff => 'SOAP::SOM')  ?  $stuff : SOAP::Deserializer->new->deserialize( $stuff )  ; 
...
#    $XML =  WSRF::SimpleSerializer->new->serialize( $som->dataof("//{$WSRF::Constants::WSA}EndpointReference")); 
    ...
    {
        my $i = 1;
	while ( $som->match("//{$WSRF::Constants::WSA}ReferenceParameters/[$i]") )
	{
	  $ref_params .= WSRF::SimpleSerializer->new->serialize( $som->dataof("//{$WSRF::Constants::WSA}ReferenceParameters/[$i]") );
	  $i++;
	}
    }
	...
	{
	  $meta_data .= WSRF::SimpleSerializer->new->serialize( $som->dataof("//{$WSRF::Constants::WSA}Metadata/[$i]") );
	  $i++;
	}
    }	   
...
sub from_envelope {
    ...
    my ($address,$ref_params,$meta_data,$XML);
    my $som = UNIVERSAL::isa($stuff => 'SOAP::SOM')  ?  
              $stuff : 
	      SOAP::Deserializer->new->deserialize( $stuff )  ; 

    
    $address = $som->match("//Body//EndpointReference/{$WSRF::Constants::WSA}Address") ?
                        $som->valueof("//Body//EndpointReference/{$WSRF::Constants::WSA}Address") :
			die "WS_Address::from_envlope No wsa:EndpointReference in Envelope Body\n";
			#  print STDERR "address= $address\n";
    
    if ( $som->match("//Body//EndpointReference/{$WSRF::Constants::WSA}ReferenceParameters") ) 
    {
        my $i = 1;
	while ( $som->match("//Body//EndpointReference/{$WSRF::Constants::WSA}ReferenceParameters/[$i]") )
	{
	  $ref_params .= WSRF::SimpleSerializer->new->serialize( $som->dataof("//Body//EndpointReference/{$WSRF::Constants::WSA}ReferenceParameters/[$i]") );
	  $i++;
	}
    }
    
    if ( $som->match("//Body//EndpointReference/{$WSRF::Constants::WSA}Metadata") ) 
    {
        my $i = 1;
	while ( $som->match("//Body//EndpointReference{$WSRF::Constants::WSA}Metadata/[$i]") )
	{
	  $meta_data .= WSRF::SimpleSerializer->new->serialize( $som->dataof("//Body//EndpointRefernce/{$WSRF::Constants::WSA}Metadata/[$i]") );
	  $i++;
	}
    }	
...
sub MessageID {
   return join '','urn:www.sve.man.ac.uk-',int(rand 100000000000) + 1,gmtime;
}
...
sub XML {
   ...
   if ( !defined $self->{_XML} )
   {
     my $XML =  '<?xml version="1.0" encoding="UTF-8"?>';
        ...
        $XML .=  $self->{_ReferenceParameters} ? $self->{_ReferenceParameters} : '';
...
sub serializeReferenceParameters {
  ...
  my $som = SOAP::Deserializer->new->deserialize( '<_foo>'.$self->{_ReferenceParameters}.'</_foo>' ) ;
...
sub die_with_Fault {
   ...
   $fault .= "<wsbf:Timestamp>".WSRF::Time::ConvertEpochTimeToString(time).
                 "</wsbf:Timestamp>";   
   
   if( defined($args{OriginatorReference}) )
   {   
      $fault .= "<wsbf:OriginatorReference>".$args{OriginatorReference}.
   ...
   if( defined($args{ErrorCode}) )
   {
     ...
     { 
        $fault .= "<wsbf:ErrorCode dialect=\"".$args{dialect}."\">".
	             $args{ErrorCode}."</wsbf:ErrorCode>";
     }
     else
     {
...
#  print STDERR "Soap Namespace= ".$soap."\n";
  ...
  {
    print STDERR "$$ ERROR in WSDL file - no ".$soap.":address element\n";
...
sub createWSAddress {
  ...
  my $response = "<wsa:EndpointReference xmlns:wsa=\"$WSRF::Constants::WSA\">";
...
sub SendSOAPToSocket {
  ...
  my $my_soap = SOAP::Lite->serializer
                          ->uri( $URI )
                          ->envelope( method => $method, @params );
  ...
  my $MyFH = IO::Socket::UNIX->new( Peer => "$rendev",
                                    Type => SOCK_STREAM,
                                    Timeout => 10 ) or 
				    die SOAP::Fault->faultcode("Container Fault")
					           ->faultstring("Container Failure - Socket problem $!");
  ...
  my $out = print $MyFH ( $request->as_string() ) or 
				    die SOAP::Fault->faultcode("Container Fault")
					           ->faultstring("Container Failure - Socket problem $!");
  ...
  if (!defined($out)) 
  { 
    print STDERR "$$ ERROR - WSRF::GSutil::SendSOAPToSocket did not get response from Socket\n";
    die SOAP::Fault->faultcode("Container Fault")
		   ->faultstring("Container Failure - Socket problem");
  }
...
sub handle {
  ...
  if (my $pid = fork) {      
    ...
    $ENV{WSA}  =  WSRF::GSutil::createWSAddress( module=> $ModuleName,
                                                 path  => $ModulePath,
                                                 ID => $resourceID );
    ...
    if( -e $rendivous)
    {      
      unlink "$rendivous" or die SOAP::Fault->faultcode("Container Fault")
			                    ->faultstring("Container Failure - Could not remove file");
    }

    print STDERR "$$ Created $resourceID rendezvous:: $rendivous\n";
    my $Handle = IO::Socket::UNIX->new(Local => "$rendivous",
  				       Type	=> SOCK_STREAM,
  				       Listen	=> SOMAXCONN )
    ...
    open(STDOUT, "> ".$ENV{WSRF_MODULES}."/logs/$resourceID.log") or 
          print STDERR "$$ WARNING: Could not open log file ".$ENV{WSRF_MODULES}."/logs/$resourceID.log in WSRF::Resource::handle\n";
    open(STDERR, ">&STDOUT"); 
    ...
    my $daemon = WSRF::Daemon->new ()
                             ->serializer (WSRF::WSRFSerializer->new)
			     ->deserializer(WSRF::Deserializer->new)
			     ->dispatch_to( "$ENV{WSRF_MODULES}"."/"."$ModulePath" )
			     ->dispatch_with({  $WSRF::Constants::WSRL 
                                                => "$ModuleName",
                                                $WSRF::Constants::WSRP
                                                => "$ModuleName",
						$WSRF::Constants::WSSG						
    ...
    print STDERR "$$ WSRF::Resource::handle caught exception: $@ - if it is \"Alarm went off\" then the WS-Resource's lifetime has expired";
    unlink($rendivous) or print STDERR "$$ WARNING: Could not remove $rendivous in WSRF::Resource::handle\n";
    print STDERR "$$ Resource Shutting Down\n";

    exit;   #should never get here!!
  } else {  #problem forking
      print STDERR "$$ ERROR: Could perform fork it start Resource in WSRF::Resource::handle\n";
      return "FAILURE";    
  }
  ...
  my $resp = WSRF::GSutil::SendSOAPToSocket($rend,
                                            $ModuleName,
					    "init",
					    @Params);
  ...
  if ( $resp->fault )
  {
    print STDERR "$$ ERROR: SOAP fault from init: ".$resp->faultstring."\n in WSRF::Resource::handle\n";
  }
...
sub DESTROY {
   ...
   if( -d $self->{_file} )
   {
     rmdir $self->{_file} or die SOAP::Fault->faultcode("Container Fault")
...
my $Insert = sub {
   ...
   if ( defined( $WSRF::WSRP::ResourceProperties{$name}) )
   {
     ...
     elsif ( $type eq "ARRAY" ) #array
     {
       ...
       push ( @{$WSRF::WSRP::ResourceProperties{$name}}, 
              $value );  
     }
     elsif ( $type ne "CODE" )
     {
       print STDERR "$$ ERROR: Property $name is a $type, only ARRAY,SCALAR and CODE are supported in WSRF::File::Insert\n";
     }    
   }
   else
   {
     print STDERR "$$ ERROR: Attempting to load property from file that has not been declared in WSRF::File::Insert\n";
   }
...
#   }
   ...
   %WSRF::WSRP::ResourceProperties = (%WSRF::WSRP::ResourceProperties, 
	                              %{$hashref->{Properties}});
   ...
   {  
     if ( WSRF::Time::ConvertStringToEpochTime($WSRF::WSRP::ResourceProperties{'TerminationTime'})
             < time )
     {
...
sub toFile {
   ...
   my $filename = ref($class) ? $class->{_path} : 
...
sub header {
 ...
 {   
     ...
     $myHeader .= "<wsa:From wsu:Id=\"From\"><wsa:EndPointReference><wsa:Address>$from</wsa:Address></wsa:EndPointReference></wsa:From>";
 }
 ...
 {
  $messageID = $envelope
    ->headerof("//{$WSRF::Constants::WSA}MessageID")
    ->value;
...
sub searchResourceProperty  {
   ...
   if ( defined( $WSRF::WSRP::ResourceProperties{$search}) )
   {
	 ...
	 {
	    $ans .= "<".$WSRF::WSRP::PropertyNamespaceMap->{$search}{prefix}.":$search ";
            #do we need to add a namespace for this property 
	    my $ns = defined($WSRF::WSRP::PropertyNamespaceMap->{$search}{namespace}) ?
	            " xmlns:".$WSRF::WSRP::PropertyNamespaceMap->{$search}{prefix}."=\"".
		    $WSRF::WSRP::PropertyNamespaceMap->{$search}{namespace}."\">" : ">";  	    
            $ans .=  $ns.
	             $WSRF::WSRP::ResourceProperties{$search}.
                     "</".$WSRF::WSRP::PropertyNamespaceMap->{$search}{prefix}.":$search>";	     
	 }
     ...
     elsif ( $type eq "ARRAY" )
     {
       ...
       {
         foreach my $entry ( @{$WSRF::WSRP::ResourceProperties{$search}} )
         {
          $ans .= "<".$WSRF::WSRP::PropertyNamespaceMap->{$search}{prefix}.":$search";
          #do we need to add a namespace for this property 
	  my $ns = defined($WSRF::WSRP::PropertyNamespaceMap->{$search}{namespace}) ?
	...
	$ans .=   $ns.                                                                        
	          $serializer->serialize($WSRF::WSRP::ResourceProperties{$search}).               
		  "</".$WSRF::WSRP::PropertyNamespaceMap->{$search}{prefix}.":$search>";
...
sub xmlizeProperties {
  ...
  my $ans = "<wsrp:ResourceProperties".
  " xmlns:wsrp=\"$WSRF::Constants::WSRP\" ".
  " xmlns:wsrl=\"$WSRF::Constants::WSRL\" ".
...
#   }
   ...
   {
      die SOAP::Fault->faultcode("setResourceproperty: No such ResourceProperty")
...
sub SetResourceProperties {
	...
	if( $Function eq "Insert" )  #an Insert
	{
	  ...
	  while ( $som->match("$base/[$jj]/[$kk]") ) 
	  {
	    ...
	    if ( ! defined($WSRF::WSRP::NotInsert{$som->dataof("$base/[$jj]/[$kk]")->name()}) )
	    {  
	...
	elsif( $Function eq "Delete" ) #a Delete
	{
	   ...
	   my $propname = $som->dataof("$base/[$jj]")
	...
	{  #something other than Insert|Update|Delete
           die SOAP::Fault->faultcode("setResourceproperty: Unkown operation")
...
sub DeleteResourceProperties {
	...
	#the property to delete is actually an attribute
	#in the delete element
        my $propname = $som->dataof("$base/[$kk]")
		    ->attr->{'ResourceProperty'};	 
	$propname =~ s/\w*://o;	    
	#delete property 
	if ( ! defined($WSRF::WSRP::NotDeletable{$propname}) ) 
...
$WSRF::WSRP::ResourceProperties{'CurrentTime'} = 
...
sub SetTerminationTime {
   ...
   if ( $time eq "" )
   {
      ...
      #disable alarm
      alarm;
      my $ans= "<wsrl:NewTerminationTime xsi:nil=\"true\" />".
               "<wsrl:CurrentTime>".
                   WSRF::Time::ConvertEpochTimeToString().
	       "</wsrl:CurrentTime>";
...
sub Destroy {  
    ...
    unlink $file or die SOAP::Fault->faultcode("Container Failure")
		                   ->faultstring("Container Failure: could not remove file");
   return WSRF::Header::header( $envelope );   
}
...
sub getID {
   ...
   #check that the resource is still alive - if TT time is not
   #set then TT is infinity
   foreach my $key ( keys %{$WSRF::MultiResourceProperties::ResourceProperties} )
   {
     if ( defined($WSRF::MultiResourceProperties::ResourceProperties->{$key}{'TerminationTime'}) &&
           ($WSRF::MultiResourceProperties::ResourceProperties->{$key}{'TerminationTime'} ne "") )
     {  
       if ( WSRF::Time::ConvertStringToEpochTime($WSRF::MultiResourceProperties::ResourceProperties->{$key}{'TerminationTime'})
             < time )
       {
          print STDERR "MultiResourceProperties Resource $key Expired\n";
          delete $WSRF::MultiResourceProperties::ResourceProperties->{$key};
       }    
     }
   }
...
my $MultisearchResourceProperty = sub {
   ...
   #Check Resource property exists, if it does it can either
   #be a simple scalar, an array or a function.
   if ( defined( $WSRF::MultiResourceProperties::ResourceProperties->{$ID}{$search}) )
   {
     #get type of property
     my $type = ref($WSRF::MultiResourceProperties::ResourceProperties->{$ID}{$search});
     if ( $type eq "" ) # if scalar
     { 
         #check if property set         
	 if ( $WSRF::MultiResourceProperties::ResourceProperties->{$ID}{$search} ne "")
	 {
	    $ans .= "<".$WSRF::MultiResourceProperties::PropertyNamespaceMap->{$search}{prefix}.":$search ";
            #do we need to add a namespace for this property 
	    my $ns = defined($WSRF::MultiResourceProperties::PropertyNamespaceMap->{$search}{namespace}) ?
	            " xmlns:".$WSRF::MultiResourceProperties::PropertyNamespaceMap->{$search}{prefix}."=\"".
		    $WSRF::MultiResourceProperties::PropertyNamespaceMap->{$search}{namespace}."\">" : ">";  	    
            $ans .=  $ns.
	             $WSRF::MultiResourceProperties::ResourceProperties->{$ID}{$search}.
                     "</".$WSRF::MultiResourceProperties::PropertyNamespaceMap->{$search}{prefix}.":$search>";	     
	 }
	 #property NOT set - is it nillable?
	 elsif ( $WSRF::MultiResourceProperties::ResourceProperties->{$ID}{$search} eq "" 
     ...
     {
       #check array is not empty - and property is nillable
       if( ! @{$WSRF::MultiResourceProperties::ResourceProperties->{$ID}{$search}} 
             && defined($WSRF::MultiResourceProperties::Nillable{$search}) )
       {           
           $ans .= "<".$WSRF::MultiResourceProperties::PropertyNamespaceMap->{$search}{prefix}.":$search";
	   my $ns = defined($WSRF::MultiResourceProperties::PropertyNamespaceMap->{$search}{namespace}) ?
       ...
       {
         foreach my $entry ( @{$WSRF::MultiResourceProperties::ResourceProperties->{$ID}{$search}} )
         {
          $ans .= "<".$WSRF::MultiResourceProperties::PropertyNamespaceMap->{$search}{prefix}.":$search";
          #do we need to add a namespace for this property 
	  my $ns = defined($WSRF::MultiResourceProperties::PropertyNamespaceMap->{$search}{namespace}) ?
     ...
     {
       $ans .=  $WSRF::MultiResourceProperties::ResourceProperties->{$ID}{$search}->();
     }
	...
	$ans .=   $ns.                                                                        
	          $serializer->serialize($WSRF::WSRP::ResourceProperties->{$ID}{$search}).               
		  "</".$WSRF::WSRP::PropertyNamespaceMap->{$search}{prefix}.":$search>";      
...
my $xmlizeProperties = sub {
  ...
  foreach my $key ( keys %{$WSRF::MultiResourceProperties::ResourceProperties->{$ID}} )
  {
...
#   }
   ...
   #check the property actually exists
   if ( defined( $WSRF::MultiResourceProperties::ResourceProperties->{$ID}{$name}) )
   {
     #check the type of the property (scalar|array)
     my $type = ref($WSRF::MultiResourceProperties::ResourceProperties->{$ID}{$name});
     if ( $type eq "" )  #scalar
     {
       $WSRF::MultiResourceProperties::ResourceProperties->{$ID}{$name} = $value;
     }
     elsif ( $type eq "ARRAY" ) #array
     {
       #add property to array
       push ( @{$WSRF::MultiResourceProperties::ResourceProperties->{$ID}{$name}}, 
              $value );  
     }  
     else #perhaps subroutine?
     {
...
sub SetResourceProperties {
	...
	if( $Function eq "Insert" )  #an Insert
	{
	  ...
	  while ( $som->match("$base/[$jj]/[$kk]") ) 
	  {
	    #print "Inserting ".$som->dataof("$base/[$jj]/[$kk]")->name()."\n";
            #insert the thing - note we pass a SOM object becasue the
            #thing could be pretty complex.
	     if ( ! defined($WSRF::MultiResourceProperties::NotInsert{$som->dataof("$base/[$jj]/[$kk]")->name()}) )
	     {     
	       $Multiinsert->(ID => $ID,
	...
	elsif( $Function eq "Delete" ) #a Delete
	{
	   ...
	   #print "Delete $propname\n";
           #delete property  
	   if ( !defined($WSRF::MultiResourceProperties::NotDeletable{$propname}) )
	   { 	   
              $Multimydelete->(ID => $ID,
	               property => $propname);
           }		       	   	    
...
sub SetTerminationTime {
   ...
   if ( $time eq "" )
   {
      $WSRF::MultiResourceProperties::ResourceProperties->{$ID}{'TerminationTime'} = "";
   ...
   #reset TerminationTime
   $WSRF::MultiResourceProperties::ResourceProperties->{$ID}{'TerminationTime'} = $time;
...
$WSRF::WSRP::NotModifiable{Entry} = 1;   #Cannot modify through SetResourceProperty 


$WSRF::WSRP::ResourceProperties{ServiceGroupEPR} = "";
$WSRF::WSRP::PropertyNamespaceMap->{ServiceGroupEPR}{prefix} = "wssg";
$WSRF::WSRP::PropertyNamespaceMap->{ServiceGroupEPR}{namespace} = $WSRF::Constants::WSSG;
$WSRF::WSRP::NotDeletable{ServiceGroupEPR}  = 1;   #Cannot delete through SetResourceProperty
$WSRF::WSRP::NotModifiable{ServiceGroupEPR} = 1;   #Cannot modify through SetResourceProperty
...
$WSRF::WSRP::InsertMap{ServiceGroupEPR} = sub {
   ...
   print STDERR "ServiceGroup WSRF::WSRP::InsertMap{ServiceGroupEPR}  called\n";
...
my $strip_old_Entries = sub {
   ...
   {     
      ...
      my $TermTime = defined ($doc->getElementsByTagName ("EntryTerminationTime")
                          ->item(0)->getFirstChild) ? 
			  $doc->getElementsByTagName ("EntryTerminationTime")
                          ->item(0)->getFirstChild->getNodeValue : "";

      next if ( ( $TermTime ne "nil") && 
...
sub createServiceGroup {
  ...
  my $wsa  =  WSRF::GSutil::createWSAddress( module=> 'ServiceGroup',
...
sub Add {
  ...
  my $mepraddress = $envelope->match("//MemberEPR//{$WSRF::Constants::WSA}Address") ?
  ...
  {
     my $i=0;
     foreach my $a ($envelope->dataof('//MemberEPR//ReferenceParameters/*'))
     {
        $i++;
        my $name  = $a->name();
        my $uri   = $a->uri();
...
#  print STDERR "Content = $content\n";
  ...
  my $sge_wsa = WSRF::GSutil::createWSAddress( module=> $WSRF::ServiceGroup::ServiceGroupEntryModule,
                                               path  => $WSRF::ServiceGroup::ServiceGroupEntryPath,
                                               ID => $ID );

  my $ans = $sge_wsa;
  $sge_wsa = "<wssg:ServiceGroupEntryEPR>$sge_wsa</wssg:ServiceGroupEntryEPR>";  
...
$WSRF::WSRP::PropertyNamespaceMap->{Content}{namespace} = $WSRF::Constants::WSSG;
$WSRF::WSRP::NotDeletable{Content}  = 1;   #Cannot delete through SetResourceProperty
$WSRF::WSRP::NotModifiable{Content} = 1;   #Cannot modify through SetResourceProperty
...
$WSRF::WSRP::PropertyNamespaceMap->{MemberEPR}{namespace} = $WSRF::Constants::WSSG;
$WSRF::WSRP::NotDeletable{MemberEPR}  = 1;   #Cannot delete through SetResourceProperty
$WSRF::WSRP::NotModifiable{MemberEPR} = 1;   #Cannot modify through SetResourceProperty
...
#      print "Length= ".$subnodes->getLength."\n";
      ...
      {
        $ResourceID = $subnodes->item(0)->getElementsByTagName("wsa:Address"); 
      }
...
#      print STDERR "$$ ResourceID = $ResourceID\n"; 
      if ( $ResourceID eq $address )  #found node we want
      {
         ...
         $Content = $doc->getElementsByTagName ("wssg:Content")->item(0)->getFirstChild->toString;
         $MEPR = $doc->getElementsByTagName ("wssg:MemberServiceEPR")->item(0)->getFirstChild->toString;	       
         $found =1;
         if ( defined($args{Destroy}) )
         {
#            print STDERR "$$ Destroying ServiceGroupEntry $ID\n";  
         ...
         {
             $doc->getElementsByTagName ("EntryTerminationTime")
                        ->item(0)->getFirstChild->setNodeValue($args{TerminationTime});
         }
         my $foo = $doc->toString;
         $foo =~ s/<\/?t>//og;
         $entry = $foo;
...
sub BEGIN {
  ...
  for my $method (qw(endpoint transport outputxml autoresult packager wsaddress)) {
    my $field = '_' . $method;
    *$method = sub {
      my $self = shift->new;
      @_ ? ($self->{$field} = shift, return $self) : return $self->{$field};
...
sub call {
  ...
  if ( !(defined $self->proxy  && UNIVERSAL::isa($self->proxy => 'SOAP::Client'))  
  ...
  {
    my $header = "<wsa:Action wsu:Id=\"Action\">".scalar($self->on_action->($serializer->uriformethod($_[0])))."</wsa:Action>";
    $header .=  "<wsa:To wsu:Id=\"To\">".$self->wsaddress->Address()."</wsa:To>";
    $header .=  "<wsa:MessageID wsu:Id=\"MessageID\">".$self->wsaddress->MessageID()."</wsa:MessageID>";
    $header .=  $self->wsaddress->serializeReferenceParameters() ? $self->wsaddress->serializeReferenceParameters() : '';
    #bug fix - John Newman
    $header .=  "<wsa:ReplyTo wsu:Id=\"ReplyTo\"><wsa:Address>$WSRF::Constants::WSA_ANON</wsa:Address></wsa:ReplyTo>";
...
$WSRF::WSS::sec_xpath=
'<XPath xmlns:wsse="'.$WSRF::Constants::WSSE.'">
...
'<XPath xmlns:wsu="'.$WSRF::Constants::WSU.'">
...
.'(//. | //@* | //namespace::*)'."[ancestor-or-self::$SOAP::Constants::PREFIX_ENV:Body]</XPath>";
...
sub load_priv_key {
    ...
    my $key_file_name = $ENV{HTTPS_KEY_FILE} ? $ENV{HTTPS_KEY_FILE} : die "No Private Key\n" ;
    open(PRIVKEY, $key_file_name) || die("Could not open file $key_file_name");
...
sub load_cert {
    ...
    my $cert_file_name = $ENV{HTTPS_CERT_FILE} ? $ENV{HTTPS_CERT_FILE} :die "No Public Key\n" ;
...
eval {require XML::CanonicalizeXML};
   die "Failed to access class XML::CanonicalizeXML: $@" if $@;
...
'<ds:SignatureMethod Algorithm="http://www.w3.org/2000/09/xmldsig#rsa-sha1"/>'; 
   ...
   {
      $for_signing .= WSRF::WSS::make_token($envelope,$WSRF::WSS::ID_Xpath{$key},$key)
           if defined($WSRF::WSS::Sign{$key});
      my $canon = XML::CanonicalizeXML::canonicalize($envelope,$WSRF::WSS::ID_Xpath{$key},'',1,0);
      $header .= defined($canon) ? $canon : "";
   }
   
   $for_signing .= WSRF::WSS::make_token($envelope,$WSRF::WSS::body_xpath,"myBody")
            if defined($WSRF::WSS::Sign{Body});


   #create a security token using the certificate 
   my $sec_token='<wsse:BinarySecurityToken xmlns:wsse="http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-secext-1.0.xsd" xmlns:wsu="http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-utility-1.0.xsd" EncodingType="http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-soap-message-security-1.0#Base64Binary" ValueType="http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-x509-token-profile-1.0#X509v3" wsu:Id="X509Token">'.$certificate.'</wsse:BinarySecurityToken>';
   if ( defined($WSRF::WSS::Sign{BinarySecurityToken}) && defined($WSRF::WSS::sec_xpath) )
   {
     $for_signing .= WSRF::WSS::make_token($sec_token,$WSRF::WSS::sec_xpath,"X509Token");
   }
   ...
   {
     $timestamp = '<wsu:Timestamp xmlns:wsu="http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-utility-1.0.xsd" wsu:Id="TimeStamp">';
     $timestamp .= '<wsu:Created>'.WSRF::Time::ConvertEpochTimeToString(time).'</wsu:Created>';
     $timestamp .= '<wsu:Expires>'.WSRF::Time::ConvertEpochTimeToString(time+(60*60)).'</wsu:Expires>';
     ...
     $for_signing .= WSRF::WSS::make_token($timestamp, $WSRF::WSS::timestamp_xpath,"TimeStamp")   if defined($WSRF::WSS::Sign{Timestamp});   
   }

   $for_signing .= '</ds:SignedInfo>';

   my $can_signed_info=XML::CanonicalizeXML::canonicalize($for_signing, $WSRF::WSS::si_xpath,'',1, 0);
...
my $extraheader = '<wsse:Security xmlns:wsu="http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-utility-1.0.xsd" 
...
$header=$extraheader.$header;
 ...
 my $Body = XML::CanonicalizeXML::canonicalize($envelope,$WSRF::WSS::body_xpath,'',1,0);
...
eval {require XML::CanonicalizeXML};
   die "Failed to access class CanonicalizeXML: $@" if $@;
...
#   print STDERR "Xpath=> $Path\n";
   my $can_token=XML::CanonicalizeXML::canonicalize($XML, $Path,'',1,0);
...
'<ds:DigestValue>'.$token_digest.'</ds:DigestValue>'.
'</ds:Reference>';    
...
$WSRF::WSS::ThingsThatShouldBeSigned{BinarySecurityToken}   = $WSRF::Constants::WSSE;
...
eval {require MIME::Base64};
   ...
   #get Security Token
   my $Token =  $envelope->match("/Envelope/Header/Security/{$WSRF::Constants::WSSE}BinarySecurityToken")   ?
                $envelope->valueof("/Envelope/Header/Security/{$WSRF::Constants::WSSE}BinarySecurityToken") :
...
#   print ">>>>Token>>>\n$Token\n<<<<Token<<<<<\n";
   ...
   my $can_signed_info=XML::CanonicalizeXML::canonicalize($envelope->raw_xml, $WSRF::WSS::si_xpath,'',1, 0);

   #get the Signature value 
   my $SignatureValue = $envelope->match("/Envelope/Header//{http://www.w3.org/2000/09/xmldsig#}SignatureValue")   ?
                        $envelope->valueof("/Envelope/Header//{http://www.w3.org/2000/09/xmldsig#}SignatureValue") :
   ...
   else
   {
    ...
    my $i = 1;
    my %SignedStuff = ();
    while(  $envelope->match("/Envelope/Header/Security/Signature/SignedInfo/[$i]")  )
    {
       my $data = $envelope->dataof("/Envelope/Header/Security/Signature/SignedInfo/[$i]");
       if( $data->name eq "Reference" )
       {  
	 my $attr = $data->attr;
         my $name = $attr->{URI};
	 my $DigestValue = $envelope->match("/Envelope/Header/Security/Signature/SignedInfo/[$i]//{http://www.w3.org/2000/09/xmldsig#}DigestValue") ?
                           $envelope->valueof("/Envelope/Header/Security/Signature/SignedInfo/[$i]//{http://www.w3.org/2000/09/xmldsig#}DigestValue") :
    ...
    {
       if( $envelope->match("/Envelope//{$WSRF::WSS::ThingsThatShouldBeSigned{$key}}$key" )  )
       {    
	  my $data = $envelope->dataof("/Envelope//{$WSRF::WSS::ThingsThatShouldBeSigned{$key}}$key");
	  my $attr = $data->attr;
   ...
   {
      my $CanonicalForm = XML::CanonicalizeXML::canonicalize($envelope->raw_xml,$WSRF::WSS::Xpath{$key},'',1,0);
   ...
   $results{Created} = $envelope->match("/Envelope/Header/Security/Timestamp/{$WSRF::Constants::WSU}Created")   ?
                       $envelope->valueof("/Envelope/Header/Security/Timestamp/{$WSRF::Constants::WSU}Created") :
		       undef;
   #print STDERR "WSRF::WSS::verify Message Created at $results{Created} (should be GMT)\n" if $results{Created};
       
   $results{Expires} = $envelope->match("/Envelope/Header/Security/Timestamp/{$WSRF::Constants::WSU}Expires")   ?
                       $envelope->valueof("/Envelope/Header/Security/Timestamp/{$WSRF::Constants::WSU}Expires") :
		       undef; 
